...
## 2026-06-23 Phase 1-Step 1-3: CanvasRendererと2Dカメラ土台

### 作業概要
CanvasBitmapをImGui上に表示するCanvasRendererと、2D撮影カメラ/カメラアニメーションの土台を追加した。
...
### 実装内容
- CanvasViewのキャンバス/画面座標変換
- FrameのLayerをCanvasBitmapへdirty時だけ再構築するCanvasRenderer
- 確定済みストロークはSDL_Textureとして表示し、入力中ストロークだけDrawListでプレビュー表示する構造
- オニオンスキン用の簡易Bitmapキャッシュ
- ShotCamera2Dのパン/ズーム/回転座標変換
- CameraAnimationのStep/Linear/Smooth補間

### 未完了
- App/UIへの接続は未実装。Step 1-4で行う。
- 実際のペン入力、レイヤーパネル、タイムライン表示は未実装。

### 次にやること
- Phase 1 Step 1-4でAppとUI配線を追加し、作画操作を実際に画面から使えるようにする。
