...
int scenePlateKindIndex(ScenePlateKind kind) noexcept
{
    switch (kind) {
...
ScenePlateKind scenePlateKindFromIndex(int index) noexcept
{
...
int scenePlateOutputModeIndex(ScenePlateOutputMode mode) noexcept
{
    switch (mode) {
...
ScenePlateOutputMode scenePlateOutputModeFromIndex(int index) noexcept
{
...
std::string defaultScenePlateName(ScenePlateKind kind, int number)
{
    return std::string(scenePlateKindJapaneseLabel(kind)) + " " + std::to_string(std::max(1, number));
}

std::string nextScenePlateId(const ScenePlateStack& stack)
{
...
int nextScenePlateZOrder(const ScenePlateStack& stack) noexcept
{
    int zOrder = 0;
...
ScenePlate makeDefaultScenePlate(const ScenePlateStack& stack, ScenePlateKind kind)
{
    ScenePlate plate;
...
void clampScenePlateSelection(const ScenePlateStack& stack, int& selectedIndex) noexcept
{
...
void assignScenePlateSequentialZOrder(ScenePlateStack& stack) noexcept
{
...
void drawScenePlatePreviewOnCanvas(const ScenePlate& plate,
                                   int timelineFrame,
                                   int canvasWidth,
                                   int canvasHeight,
...
void drawScenePlatePreviewStackOnCanvas(const ScenePlateStack& stack,
                                        int timelineFrame,
                                        int canvasWidth,
                                        int canvasHeight,
                                        const std::filesystem::path& projectFolder,
...
void App::drawCanvasArea(float rightWidth)
{
    ...
    // Timesheet Rebuild Step 7.15:
    // Scene Plateの画像パスから実画像を読み込み、キャンバス背景として表示する。
    // 読み込み失敗時は従来どおりダミー矩形へフォールバックする。
    ...
    if (scenePlateCanvasPreviewEnabled_) {
        ...
        drawScenePlatePreviewStackOnCanvas(
            workingScenePlates_,
